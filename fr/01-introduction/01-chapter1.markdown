...
Sur les systèmes Windows, Git recherche le fichier `.gitconfig` dans le répertoire `%USERPROFILE%` (généralement `C:\Documents and Settings\%USERNAME%` ou `C:\Users\%USERNAME%` selon la version).
