...
vous devriez utiliser Git avec la console (*shell*) fournie par msysGit (style Unix), car elle permet d'utiliser les lignes de commandes complexes données dans ce livre.
Si vous devez, pour une raison quelconque, utiliser la console native de Windows (console système), vous devez utiliser des guillemets au lieu des apostrophes pour délimiter les paramètres avec des espaces.
