...
On Windows systems, Git looks for the `.gitconfig` file in the `%USERPROFILE%` directory (`C:\Documents and Settings\%USERNAME%` or `C:\Users\%USERNAME%` for most people, depending on version). It also still looks for /etc/gitconfig, although it’s relative to the MSys root, which is wherever you decide to install Git on your Windows system when you run the installer.
